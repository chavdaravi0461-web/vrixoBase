...
function GoogleButton({ isLoading, onError }: OAuthButtonProps) {
  ...
  return (
    <Button
      variant="outline"
      className="w-full h-10 gap-2 text-sm font-normal"
      onClick={handleClick}
      disabled={loading}
    >
      {loading ? (
...
function GitHubButton({ isLoading, onError }: OAuthButtonProps) {
  const [localLoading, setLocalLoading] = useState(false);
  const loading = isLoading || localLoading;
